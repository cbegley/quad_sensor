...
//static final int chipSelectPin = 19; // SPI0
//static final int chipSelectPin = 15; // SPI1
//static final int chipSelectPin = 76;
...
void setup() {
  size(600,400);
  printArray(SPI.list());
  //tmag = new SPI(SPI.list()[0]); // SPI0
  tmag = new SPI(SPI.list()[1]); // SPI1
  tmag.settings(500000, SPI.MSBFIRST, SPI.MODE0);
  
  //spi[0] = new SPI(SPI.list()[0]);
  //spi[1] = new SPI(SPI.list()[1]);
  ...
  //GPIO.pinMode(chipSelectPin, GPIO.OUTPUT);
...
void keyPressed() {
 //if (key == '1') selectOutput(0);
 //if (key == '2') selectOutput(1);
 //if (key == '3') selectOutput(2);
 //if (key == '4') selectOutput(3);
 if (key == '1') {
   currentSensor = 0;
   pinChange = true;
 }
